...
def fast_adapt(batch, learner, generator, optimizer2 , train_ratio ,loss, loss1 ,adaptation_steps, shots, ways, device, test):
    ...
    en = torch.tile(encoding , (adaptation_data.shape[0],1))
    ...
    for step in range(adaptation_steps):
        ...
        if alpha :
          ...
          for i in range(train_ratio):
            ...
            fake_data = generator(inp)
          
            _ , last_logit = learner(fake_data.reshape(-1,16))

            G_label = torch.zeros((ways*shots)).to(device)

            G_loss = loss1(last_logit.reshape(ways*shots) , G_label )
        ...
        G_label = torch.ones((ways*shots)).to(device)

        fake_loss = loss1(last_logit.reshape(ways*shots) , G_label )
